...
function Signup() {
  ...
  return (
   <div className="bg-[#0A0A0A] min-h-screen text-white font-sans relative overflow-hidden">
      ...
      <h2 className="text-3xl font-bold w-full">Initialize Registration</h2>
        ...
        <p className="text-[#BACCB0] pt-2">Initialize session to access the roodmap core.</p>
        <form id="Form-Login" className="flex flex-col justify-start mt-5">
  <div className="relative w-90 group">
    <label htmlFor="Email" className="text-[#BACCB0] text-sm font-medium">IDENTIFIER</label>
  ...
  <div className="relative w-90 group mt-5">
    <label htmlFor="Password" className="text-[#BACCB0] text-sm font-medium">SECRET_KEY</label>
  ...
  <div className="relative w-90 group mt-5 mb-3">
      <label htmlFor="name" className="text-[#BACCB0] text-sm font-medium">IDENTITY_NAME</label>
      
      <FaUser className="absolute top-[47px] left-3 text-[#00FFFF] group-focus-within:text-[#37FF14] text-xl transition-colors duration-300 z-10" />
    ...
    <p className='mt-5'>Already have an access? Login <Link to="/login" className="mb-3 hover:text-[#37FF14] duration-300">here</Link></p>
  <button className="py-3 px-2 bg-[#39FF14] mt-5 text-black cursor-pointer border hover:bg-white/5 hover:border-[#39FF14] hover:text-white duration-300">EXECUTE_REGISTRATION</button>
</form>
      </div>
      </div>
  ...
  <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
    ...
    <div className="flex gap-8">
      <Link to="/terms" className="text-white/50 hover:text-[#39FF14] text-sm transition-colors" target="_blank">Privacy</Link>
      <Link to="/terms"  className="text-white/50 hover:text-[#39FF14] text-sm transition-colors" target="_blank">Terms</Link>
    </div>
